...
class _HistoryScreenState extends ConsumerState<HistoryScreen>
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatisticsCards(historyState.history, userPrefs.dailyGoalMl, colorScheme),
            const SizedBox(height: 24),

            // Insights Section
            _buildInsightsSection(historyState.history, userPrefs.dailyGoalMl, colorScheme),
  ...
  Widget _buildStatisticsCards(List<DailySummary> history, double goalMl, ColorScheme colorScheme) {
    final today = DateTime.now();
    final todayIntake = history.where((summary) =>
        summary.date.day == today.day &&
        summary.date.month == today.month &&
        summary.date.year == today.year).fold(0.0, (sum, summary) => sum + summary.totalIntakeMl);

    final weekIntake = history.where((summary) {
      final weekStart = today.subtract(Duration(days: today.weekday - 1));
      return summary.date.isAfter(weekStart.subtract(const Duration(days: 1)));
    }).fold(0.0, (sum, summary) => sum + summary.totalIntakeMl);

    final avgDailyIntake = history.isEmpty 
        ? 0.0 
        : history.fold(0.0, (sum, summary) => sum + summary.totalIntakeMl) / history.length;
  ...
  Widget _buildInsightsSection(List<DailySummary> history, double goalMl, ColorScheme colorScheme) {
    if (history.isEmpty) {
  ...
  int _calculateStreak(List<DailySummary> history, double goalMl) {
    ...
    for (int i = 0; i < sortedHistory.length; i++) {
      ...
      final summary = sortedHistory.where((s) =>
          s.date.day == checkDate.day &&
          s.date.month == checkDate.month &&
          s.date.year == checkDate.year).firstOrNull;
  ...
  List<String> _generateInsights(List<DailySummary> history, double goalMl) {
    ...
    // Calculate average
    final avgIntake = history.fold(0.0, (sum, s) => sum + s.totalIntakeMl) / history.length;
    final goalPercentage = (avgIntake / goalMl * 100).round();
    
    if (goalPercentage >= 100) {
      insights.add('Great job! You\'ve been consistently meeting your hydration goals.');
    } else if (goalPercentage >= 80) {
      insights.add('You\'re doing well! You\'re at $goalPercentage% of your daily goal on average.');
    } else {
      insights.add('Try to increase your water intake. You\'re at $goalPercentage% of your goal.');
    }
    ...
    if (weekdayIntake.isNotEmpty) {
      final weekdayAvgs = weekdayIntake.map((day, intakes) =>
          MapEntry(day, intakes.fold(0.0, (a, b) => a + b) / intakes.length));
      
      final bestDay = weekdayAvgs.entries.reduce((a, b) => a.value > b.value ? a : b);
      final dayNames = ['', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
      
      insights.add('Your best hydration day is usually ${dayNames[bestDay.key]}.');
    }
