...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  bool _isRecommendationBannerDismissed = false;
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await notifier.resetDailyIntake();
              if (context.mounted) {
  ...
  Widget _buildRecommendationBanner(BuildContext context, WidgetRef ref) {
    ...
    final isUsingRecommended = ref
        .read(userPreferencesProvider.notifier)
        .isUsingRecommendedGoal();

    // Don't show banner if user is already using recommended intake or if dismissed
    if (isUsingRecommended || _isRecommendationBannerDismissed) {
          ...
          Row(
            children: [
              ...
              IconButton(
                onPressed: () {
                  setState(() {
                    _isRecommendationBannerDismissed = true;
                  });
                },
                icon: Icon(
                  Icons.close,
          ...
          SizedBox(
            ...
            child: FilledButton.tonal(
              onPressed: () {
                ref
                    .read(userPreferencesProvider.notifier)
                    .updateDailyGoal(recommendedIntake);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
